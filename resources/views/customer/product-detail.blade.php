...
@section('content')
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10" x-data="{
        ...
        <nav class="flex text-sm text-slate-500 mb-8 opacity-0 translate-y-4 transition-all duration-700"
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                    <div class="flex items-center">
                        <svg class="w-4 h-4 text-slate-400 mx-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                        </svg>
                        <a href="{{ route('home') }}#produk"
                            class="hover:text-blue-600 font-medium transition-colors">Produk</a>
                    </div>
                </li>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
            ...
            <div class="lg:col-span-5 opacity-0 translate-y-8 transition-all duration-700 delay-100"
                :class="loaded ? '!opacity-100 !translate-y-0' : ''">
                <div class="sticky top-32">
                    <div
                        class="bg-white rounded-[2.5rem] p-4 shadow-xl shadow-slate-200/40 border border-slate-100 relative group overflow-hidden">
                        ...
                        <div class="absolute top-8 left-8">
                            @if ($product->stock > 0)
                                <span
                                    class="bg-white/90 backdrop-blur-md text-slate-900 font-black text-xs px-4 py-2 rounded-full shadow-lg border border-white">
                                    Sisa {{ $product->stock }} Unit
                                </span>
                            @else
                                <span
                                    class="bg-red-500/90 backdrop-blur-md text-white font-black text-xs px-4 py-2 rounded-full shadow-lg border border-red-400">
                                    Habis Terjual
                                </span>
                            @endif
                        </div>
            ...
            <div class="lg:col-span-7 opacity-0 translate-y-8 transition-all duration-700 delay-200"
                :class="loaded ? '!opacity-100 !translate-y-0' : ''">

                {{-- Badge Kategori --}}
                <div
                    class="mb-3 inline-flex items-center gap-2 px-3 py-1 bg-indigo-50 border border-indigo-100 rounded-lg shadow-sm">
                    <span class="text-indigo-500 text-sm">{{ $product->category->icon ?? '📦' }}</span>
                    <span
                        class="text-indigo-700 text-xs font-black uppercase tracking-widest">{{ $product->category->name ?? 'Uncategorized' }}</span>
                </div>

                <h1 class="text-4xl md:text-4xl font-extrabold text-slate-900 mb-4 leading-tight">
                    {{ $product->name }}
                </h1>

                <div class="flex items-end gap-4 mb-8">
                    <span
                        class="text-4xl md:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">
                        Rp {{ number_format($product->price, 0, ',', '.') }}
                    </span>
                </div>

                <p class="text-slate-600 text-sm leading-relaxed mb-2 border-b border-slate-100 pb-4">
                ...
                <form action="{{ route('cart.store') }}" method="POST" class="space-y-8">
                        ...
                        <div>
                            ...
                            <div class="flex flex-wrap gap-3">
                                @foreach (explode(',', $product->variants) as $variant)
                                    <button type="button" @click="selectedVariant = '{{ trim($variant) }}'"
                                        :class="selectedVariant === '{{ trim($variant) }}' ?
                            ...
                            <p x-show="!selectedVariant"
                                class="text-amber-500 text-xs font-bold mt-2 flex items-center gap-1 animate-pulse">
                    ...
                    <div class="flex flex-col sm:flex-row gap-16">
                        <div class="w-full sm:w-1/3">
                            ...
                            <div
                                class="flex items-center justify-between bg-slate-50 border border-slate-200 rounded-2xl p-2">
                                <button type="button" @click="if(qty > 1) qty--"
                                    class="w-10 h-10 flex items-center justify-center text-slate-500 hover:text-slate-900 hover:bg-white rounded-xl transition-all shadow-sm disabled:opacity-50">
                                ...
                                <button type="button" @click="if(qty < maxStok) qty++"
                                    class="w-10 h-10 flex items-center justify-center text-slate-500 hover:text-slate-900 hover:bg-white rounded-xl transition-all shadow-sm">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="w-full sm:w-1/2 flex items-end">
                            @if ($product->stock > 0)
                                ...
                                @else
                                    <button type="button" @click="modalLogin = true"
                            ...
                            @else
                                <button type="button" disabled
                                    class="w-full h-[60px] bg-slate-200 text-slate-500 rounded-2xl font-bold text-lg flex items-center justify-center cursor-not-allowed">
                                    Stok Habis
                                </button>
                            @endif
                        </div>
                    ...
                    <div class="grid grid-cols-2 gap-4 pt-6 mt-6 border-t border-slate-100">
                        <div class="flex items-center gap-3 text-slate-600">
                            <div class="bg-blue-50 text-blue-600 p-2.5 rounded-xl"><svg class="w-5 h-5" fill="none"
